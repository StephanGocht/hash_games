...
class VarBits(object):
    ...
    def rightrotate(self, n, value):
        nBits = len(n)
        return n[nBits - value:] + n[:nBits - value]

    def rightshift(self, n, value):
        nBits = len(n)
        return ([self.zero] * value) + n[:nBits - value]



class CNFBits(VarBits):
    ...
    def getAdderMap(self):
        if self.adderMap is None:
            self.adderMap = dict()
            for i,j in [(2,2), (3,2), (4,3), (5,3), (6,3), (7,3)]:
                fileName = "data/halfadder-%i-%i.out.txt"
